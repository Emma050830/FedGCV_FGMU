...
from FGMU.task.base import BaseTask
from FGMU.utils.basic_utils import extract_floats, idx_to_mask_tensor, mask_tensor_to_idx
from os import path as osp
from FGMU.utils.metrics import compute_supervised_metrics
import os
import torch
from FGMU.utils.task_utils import load_node_edge_level_default_model
import pickle
import numpy as np
from FGMU.utils.privacy_utils import clip_gradients, add_noise
from FGMU.data.data_processing import processing    
...
class NodeClsTask(BaseTask):
    ...
    def load_train_val_test_split(self):
        ...
        if self.client_id is None and len(self.args.dataset) == 1: # server
            ...
            def _mask_matches_graph(mask, num_nodes: int) -> bool:
                try:
                    return hasattr(mask, 'numel') and int(mask.numel()) == int(num_nodes)
                except Exception:
                    return False
            ...
            use_cached = (
                osp.exists(train_path) and osp.exists(val_path) and osp.exists(test_path)
                and osp.exists(glb_train_path) and osp.exists(glb_val_path) and osp.exists(glb_test_path)
            )
            if use_cached:
                # Mask is a Tensor: explicitly set weights_only=True to avoid FutureWarning
                ...
                # Robustness: cached masks may belong to a different graph size (e.g., virtual client regenerated).
                if not (
                    _mask_matches_graph(train_mask, num_nodes)
                    and _mask_matches_graph(val_mask, num_nodes)
                    and _mask_matches_graph(test_mask, num_nodes)
                ):
                    use_cached = False
