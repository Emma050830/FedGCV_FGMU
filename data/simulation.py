...
try:
    import pymetis as metis  # type: ignore
...
def subgraph_fl_louvain_plus(args, global_dataset):
    ...
    louvain = Louvain(modularity='newman', resolution=args.louvain_resolution, return_aggregate=True) # larger resolution -> more communities (finer granularity)
...
def subgraph_fl_metis_plus(args, global_dataset):
    ...
    if metis is None:
        raise ModuleNotFoundError(
            "Missing optional dependency 'pymetis'. Install it to use 'subgraph_fl_metis'/'subgraph_fl_metis_plus'. "
            "Example: pip install pymetis"
        )
