import torch
import random
from FGMU.data.distributed_dataset_loader import FGLDataset
from FGMU.utils.basic_utils import load_client, load_server
from FGMU.utils.logger import Logger
...
class FGLTrainer:
    ...
    def aggregate_models(self, client_models):
        # Assume client_models is List[nn.Module]
        global_state_dict = client_models[0].state_dict()
        for key in global_state_dict.keys():
            params = [m.state_dict()[key] for m in client_models]
            avg_param = torch.stack(params, dim=0).mean(dim=0)
            # Add Gaussian noise
            noise_scale = self.args.dp_noise_scale if hasattr(self.args, "dp_noise_scale") else 1e-2
            avg_param = add_gaussian_noise(avg_param, noise_scale)
